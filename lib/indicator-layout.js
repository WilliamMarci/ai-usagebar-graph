...
export function sourceSupported(vendor, source) {
    ...
    if (vendor === 'deepseek')
        return ['session', 'weekly', 'monthly', 'peak'].includes(quota);
...
function layer(raw = {}) {
    return {
        ...
        labelPosition: raw.labelPosition === 'start' ? 'start' : 'end',
        labelFontSize: number(raw.labelFontSize, 7, 5, 12),
        markerEnabled: raw.markerEnabled === true,
        markerColor: /^#[0-9a-f]{6}$/i.test(raw.markerColor ?? '') ? raw.markerColor : '',
    };
}
...
export function parseIndicatorLayout(text) {
    ...
    try {
        ...
        return raw.slice(0, 12).map((item, i) => {
            const type = ['ring', 'bar', 'text', 'heatmap', 'timer'].includes(item?.type) ? item.type : 'ring';
            ...
            if (type === 'text') {
                ...
                out.timerMode = item.timerMode === 'pomodoro' ? 'pomodoro' : 'quota';
                out.vendor = VISUAL_VENDORS.includes(item.vendor) ? item.vendor : 'active';
                out.source = SOURCES.includes(item.source) ? item.source : 'session_reset';
                out.fontSize = number(item.fontSize, 10, 6, 24);
                out.prefix = String(item.prefix ?? '');
                out.durationMinutes = number(item.durationMinutes, 25, 1, 180);
                out.startedAt = number(item.startedAt, Date.now(), 0, Number.MAX_SAFE_INTEGER);
            } else {
                ...
                if (type === 'ring') {
                    ...
                    out.center = {enabled: c.enabled === true, vendor: VISUAL_VENDORS.includes(c.vendor) ? c.vendor : 'active',
                        ...
                        template: String(c.template ?? ''), timeFormat: String(c.timeFormat ?? '{compact}')};
                } else {
...
export function newItem(type, id = `${Date.now()}`) {
    ...
    if (type === 'timer')
        return {id, type, timerMode: 'quota', vendor: 'active', source: 'session_reset', fontSize: 10,
            prefix: '', durationMinutes: 25, startedAt: Date.now()};
    if (type === 'bar')
    ...
    return {id, type: 'ring', size: 24, layerGap: 0.5,
        center: {enabled: true, vendor: 'active', source: 'session', mode: 'remaining', fontSize: 8,
            template: '', timeFormat: '{compact}'}, layers: [layer()]};
}
