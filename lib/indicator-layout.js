...
export const DEFAULT_LAYOUT = Object.freeze([
    {
        ...
        center: {enabled: true, source: 'session', mode: 'remaining', fontSize: 8},
        layers: [
            {source: 'session', mode: 'remaining', color: '#2ec27e', tiered: true, thickness: 2.5},
            {source: 'weekly', mode: 'remaining', color: '#3584e4', tiered: true, thickness: 2},
        ],
    },
    {
        id: 'reset', type: 'ring', size: 24,
        center: {enabled: false, source: 'session_reset', mode: 'remaining', fontSize: 8},
        layers: [
            {source: 'session_reset', mode: 'remaining', color: '#2ec27e', tiered: false, thickness: 2.5},
            {source: 'weekly_reset', mode: 'remaining', color: '#3584e4', tiered: false, thickness: 2},
        ],
    },
]);
    ...
    try {
        ...
        return raw.slice(0, 12).map((item, i) => {
            ...
            if (type === 'text') {
                ...
                if (type === 'ring') {
                    const c = item.center ?? {};
                    out.center = {enabled: c.enabled === true, source: SOURCES.includes(c.source) ? c.source : 'session',
...
export function newItem(type, id = `${Date.now()}`) {
    ...
    if (type === 'bar')
        return {id, type, orientation: 'horizontal', length: 36, size: 20, layers: [layer()]};
    return {id, type: 'ring', size: 24, center: {enabled: true, source: 'session', mode: 'remaining', fontSize: 8}, layers: [layer()]};
}
