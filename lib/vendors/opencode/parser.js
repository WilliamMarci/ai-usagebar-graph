...
export function parseUsage(bytesOrText) {
    ...
    if (!usage || !usage.rolling || !usage.weekly || !usage.monthly)
        throw new Error('OpenCode Go usage response is missing rolling, weekly, or monthly data');
    return {
        plan: 'OpenCode Go',
...
export function parseTokscaleUsage(row) {
    const metrics = Array.isArray(row?.metrics) ? row.metrics : [];
    const find = label => metrics.find(metric => metric.label?.toLowerCase() === label.toLowerCase());
    const normalized = metric => ({percent: metric?.used_percent ?? 0, resetsAt: metric?.resets_at, status: 'ok'});
    return {
        plan: row?.plan ? `OpenCode ${row.plan}` : 'OpenCode Go',
        session: window(normalized(find('Rolling')), ROLLING_MS),
        weekly: window(normalized(find('Weekly')), WEEKLY_MS),
        monthly: window(normalized(find('Monthly')), MONTHLY_MS),
    };
}
